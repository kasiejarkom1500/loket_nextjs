...
const PAGE_SIZE = 10;
const emptyForm = { code: "", name: "", color: defaultColor };
...
export default function AdminServicesPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return services.filter((s) => s.code.toLowerCase().includes(needle) || s.name.toLowerCase().includes(needle));
  }, [services, query]);
  ...
  const openAdd = () => { setEditingId(null); setForm(emptyForm); setStatus(null); setModalOpen(true); };
  const openEdit = (s: Service) => { setEditingId(s.id); setForm({ code: s.code, name: s.name, color: s.color ?? defaultColor }); setStatus(null); setModalOpen(true); };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch("/api/admin/services", {
      ...
      body: JSON.stringify({ id: editingId, code: form.code, name: form.name, color: form.color }),
    });
    setSaving(false);
    if (!res.ok) { const d = await res.json().catch(() => ({})); setStatus(d.error ?? "Gagal menyimpan layanan."); return; }
    const d = await res.json();
    if (editingId) { setServices((p) => p.map((i) => (i.id === editingId ? d.service : i))); } else { setServices((p) => [...p, d.service]); }
    closeModal();
  ...
  const handleDelete = async (s: Service) => {
    ...
    if (!res.ok) { const d = await res.json().catch(() => ({})); setStatus(d.error ?? "Gagal menghapus layanan."); return; }
    ...
    <>
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-bold text-zinc-900">Kelola Layanan</h1>
        </div>
      ...
      <section className="rounded-2xl border border-white/70 bg-white/80 p-5 shadow-sm">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="relative w-full max-w-sm">
            ...
            <input value={query} onChange={(e) => setQuery(e.target.value)} className="w-full rounded-xl border border-zinc-200 bg-white py-2.5 pl-10 pr-4 text-sm text-zinc-900 outline-none transition focus:border-blue-400 focus:ring-2 focus:ring-blue-100" placeholder="Cari kode atau nama..." />
          </div>
          <p className="text-xs text-zinc-400">{filtered.length} layanan</p>
        </div>
        {status && !modalOpen ? (
      ...
      <section className="rounded-2xl border border-white/70 bg-white/80 shadow-sm">
        <div className="flex items-center justify-between border-b border-zinc-100 px-6 py-4">
          <h2 className="text-sm font-bold text-zinc-900">Daftar Layanan <span className="ml-1 rounded-full bg-zinc-100 px-2.5 py-0.5 text-xs font-semibold text-zinc-500">{filtered.length}</span></h2>
          ...
          <table className="min-w-full">
            <thead>
              <tr className="border-b border-zinc-100 bg-zinc-50/60">
                <th className="px-6 py-3 text-left text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-400">Kode</th>
                <th className="px-4 py-3 text-left text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-400">Nama</th>
                <th className="px-4 py-3 text-left text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-400">Warna</th>
            ...
            <tbody className="divide-y divide-zinc-50">
              {paginated.length ? paginated.map((s) => (
                <tr key={s.id} className="group transition-colors hover:bg-blue-50/40">
                  <td className="px-6 py-3.5 text-sm font-bold text-zinc-900">{s.code}</td>
                  <td className="px-4 py-3.5 text-sm text-zinc-700">{s.name}</td>
                  <td className="px-4 py-3.5">
                    <div className="flex items-center gap-2.5">
                      <span className="h-6 w-6 rounded-full border border-zinc-200 shadow-sm" style={{ backgroundColor: s.color ?? "#e5e7eb" }} />
                      <span className="font-mono text-xs text-zinc-500">{(s.color ?? "-").toUpperCase()}</span>
                    </div>
                  </td>
                  <td className="sticky right-0 bg-white px-4 py-3.5 text-right group-hover:bg-blue-50/40">
                    <div className="flex items-center justify-end gap-2">
                      <button type="button" onClick={() => openEdit(s)} className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-zinc-200 bg-white px-3 text-xs font-semibold text-zinc-600 shadow-sm transition hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700">
                      ...
                      <button type="button" onClick={() => handleDelete(s)} className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-rose-200 bg-rose-50 px-3 text-xs font-semibold text-rose-600 shadow-sm transition hover:border-rose-300 hover:bg-rose-100">
                ...
                <tr><td className="px-6 py-12 text-center text-sm text-zinc-400" colSpan={4}>Belum ada layanan.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-auto bg-black/40 p-6 pt-[8vh] backdrop-blur-sm">
          <form onSubmit={handleSubmit} className="w-full max-w-md rounded-2xl border border-white/80 bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-zinc-100 px-7 py-5">
              <h2 className="text-base font-bold text-zinc-900">{editingId ? "Edit Layanan" : "Tambah Layanan Baru"}</h2>
            ...
            <div className="px-7 py-6">
              <div className="grid gap-4">
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold uppercase tracking-[0.2em] text-zinc-400">Kode Layanan</label>
                  <input value={form.code} onChange={(e) => setForm((f) => ({ ...f, code: e.target.value }))} className="rounded-xl border border-zinc-200 bg-white px-4 py-2.5 text-sm text-zinc-900 outline-none transition focus:border-blue-400 focus:ring-2 focus:ring-blue-100" placeholder="Contoh: A" required />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold uppercase tracking-[0.2em] text-zinc-400">Nama Layanan</label>
                  <input value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} className="rounded-xl border border-zinc-200 bg-white px-4 py-2.5 text-sm text-zinc-900 outline-none transition focus:border-blue-400 focus:ring-2 focus:ring-blue-100" placeholder="Contoh: Layanan Publik" required />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold uppercase tracking-[0.2em] text-zinc-400">Warna</label>
                  <div className="flex items-center gap-3">
                    <input type="color" value={form.color} onChange={(e) => setForm((f) => ({ ...f, color: e.target.value }))} className="h-10 w-16 cursor-pointer rounded-xl border border-zinc-200 bg-white px-1.5 py-1" />
                    <span className="rounded-lg border border-zinc-100 bg-zinc-50 px-3 py-1.5 font-mono text-xs text-zinc-600">{form.color.toUpperCase()}</span>
                  </div>
                </div>
              </div>
              {status && modalOpen ? (
